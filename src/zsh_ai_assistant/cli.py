...
def generate_command(prompt: str, test_mode: bool = False) -> str:
    ...
    if not config.is_valid:
        print('Error: Invalid AI configuration. Please set OPENAI_API_KEY and OPENAI_BASE_URL', file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'command':
        ...
        print('  history-to-json - Convert history to JSON (reads from stdin)', file=sys.stderr)
        sys.exit(1)
